...
class ArrayListTest {
	...
	void setUp() {
		...
		for( int i = 0; i < numbers.length; i++) {
			list.add(numbers[i]);  //только для ar.list система авт(реалокейт*) увелич массив
		}
	}
	...
	void testRemovePattern() {
		// Passing the Integers this way to make sure the Pattern Function is called - not the Index one
		assertEquals(true, list.remove(new Integer(10)));
		assertEquals(false, list.remove(new Integer(999)));
	}
	...
	void testtoArray() {
		...
		Object[] actual = list.toArray(new Integer[8]); //Object parent of all
